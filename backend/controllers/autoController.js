...
exports.register = async (req, res) => {
  try {
    ...
    res.status(201).json({ message: 'Kont kreye avèk siksè', id_utilisateur: result.insertId });

  } catch (err) {
    res.status(500).json({ message: 'Erè sèvè', erreur: err.message });
  }
};
...
exports.login = async (req, res) => {
  try {
    ...
    if (rows.length === 0) {
      return res.status(401).json({ message: 'Email oswa mo de pas envalid' });
    }
